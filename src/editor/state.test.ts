...
import { editorReducer, initEditor } from "./reducer";
import { isDirty, toLesson } from "./state";
...
const apply = (state: EditorState, ...actions: EditorAction[]) =>
...
describe("parts", () => {
	it("move with their key and remark", () => {
		const state = apply(
			start(),
			{ type: "noteChanged", index: 0, note: "проще" },
			{ type: "sectionMoved", index: 0, delta: 1 }
		);
...
describe("issues", () => {
	it("stay on text edits and go stale on structural ones", () => {
		expect(
			apply(start(), { type: "fieldsChanged", fields: { topic: "Фрукты" } }).issues
		).toEqual([issue]);
...
it("tracks unsaved changes against the last save", () => {
	...
	const saved = apply(edited, {
		type: "saved",
		result: { lesson: toLesson(edited), issues: [] },
	});
...
it("clears remarks on a rewrite, which can be undone", () => {
	...
	const revised = apply(noted, {
		type: "revised",
		result: { lesson: rewritten, issues: [issue] },
	});
